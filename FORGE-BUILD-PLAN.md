...
Registry loader: PLT-6 deploy step loads `agents/registry.yaml` into Firestore `agent_registry`; Orchestrator discovers specialists by capability + APPROVED definition status, never hard-coded addresses (REG-1..REG-3). Handlers follow the claim/lease pattern — model/tool calls NEVER inside a transaction callback (`bus.process_message`).
**Exit: Orchestrator assigns via registry discovery; both agents return contract-valid outputs on emulators.**
...
### Day 6 — Wed Aug 26 *(v1.2 additions: REG-4 catalog, CI-9, DAT-1 deploy config)* · Human Gate, Dashboard, IAM · **GEMMA GATE**
...
v1.2 additions due today: read-only Agent Catalog on the dashboard (REG-4); approval surface writes `approval_decision.v2` records via `state.record_approval_decision` with approver identity from the authenticated IAP principal (HUM-1); CI-9 config gate — every deployed service has exactly one valid registry definition and every resource declaration validates against `infra/residency.yaml` (DAT-1), including the Pub/Sub regional endpoint + message-storage policy `forge_common.pubsub` now applies outside the emulator.

### Day 7 — Thu Aug 27 *(v1.2 additions: OBS-1/2; emulator ≠ deployed)* · Full Spine + CI Complete
...
Deferred hardening (entrant notes, Day 4 reviews): make redundant blocking idempotent on the remaining unconditional escalation paths (NoCapableAgent/reasoning). v1.2 additions due today: OBS-1/OBS-2 — ADK OTel instrumentation to Cloud Trace, W3C context authoritative across Pub/Sub hops, metadata-only spans; verify one spine run renders as a single waterfall. Deployment-tier verification (Lane 2, CI-6) that emulators cannot prove: live Cloud Run + IAM per-role denial, Pub/Sub regional endpoint + storage policy + actual DLQ forwarding after 5 nacks, Model Armor templates in-region, Cloud Trace ingest — all on project `forge-agentic-0821-6418`.
...
- Plan v1.1 (2026-08-21) — Synced to ratified spec v1.2 (21-day skip, `v1.2-demo` tag); entry basis corrected to individual/solo; v1.2 additions scheduled: REG-1..3 → Day 3, REG-4 + CI-9 + DAT-1 deploy config + HUM-1 authenticated approval surface → Day 6, OBS-1/2 + deployment-tier Lane 2 verification → Day 7, SUB-8 text description → Days 9–10 (existing item, now traceable to SUB-8). Day 2 emulator gate satisfied 2026-08-21 (PR #3); claim/lease added before agent work (PR #4).
