...
Version 1.3 · Baseline for All Things Agentic Hackathon submission (v1.2 ratified 2026-08-21; v1.3 amendment drafted 2026-08-23 per entrant directive — ratification at entrant PR review)
...
This document is the governing baseline. All build sessions, human or AI-assisted, conform to it. Changes require a version bump and a one-line rationale in the Change Log. If code and this document disagree, this document wins until formally amended. Scope is FROZEN at v1.3. This is the build baseline.
...
- **HUM-3** *(new in v1.3)*: The console SHALL provide governed operator controls: workflow initiation (injecting the `nmc_event` the field would emit, enqueued atomically with the workflow state document), workflow cancellation (a terminal `CANCELLED` state reachable from every non-terminal state and from nowhere else), Logical Clock advance, and anomaly injection (relaying the poisoned vendor bulletin to the deployed Cyber Trust quarantine path; audited agent-instance failure induction and restoration as REG-5 instance transitions). Every operator action SHALL derive identity from the authenticated principal per HUM-1, SHALL be recorded in the audit trail with that identity, and SHALL route through the sanctioned transactional writers. In-flight bus traffic for a cancelled workflow SHALL drain as inert stale no-ops and SHALL never dead-letter — audited on every model-bearing or transition-bearing path (trigger, plan, verdict, specialist assignment, safety validation); the failure and sourcing-report paths consume with zero writes per their pre-existing stale-ownership conventions. Operator failure induction SHALL NOT fabricate `agent_failure_event` messages — failure detection remains with its real producers. *Verify: test + demonstration.*
...
Pipeline: GitHub Actions. No direct commits to `main`; PRs only, including from AI-assisted sessions. Validation runs on every PR; deployment runs keylessly (WIF) on acceptance-candidate tags and manual dispatch — deliberately NOT on every merge, so documentation merges cannot churn the frozen demo runtime. *(v1.3: trigger aligned to the tag series.)*
...
**Lane 2 — Staging gate (on acceptance-candidate tags / manual dispatch):**
...
- **DFT-4**: After the dress-rehearsal recording (Day 8), `main` is frozen except for demo-spine defects and formally ratified amendments. The freeze baseline was tagged `v1.2-demo`; each post-freeze change advances the tag series (currently `v1.3-demo.N`) and re-runs the CI-8 acceptance before recording (the reset rule). *(v1.3: wording updated to the tag series; the freeze discipline is unchanged.)*
...
OUT of scope: multi-vehicle concurrent recoveries, real supplier APIs, authentication/multi-tenancy beyond IAM scoping, mobile UI, configurable fleets, tamper-proof logging, and any CALM-CFR+ solver integration. The dashboard is read-only except for approve/reject controls (HUM-1) and the governed operator controls (HUM-3). *(v1.2 additions:)* Also OUT: trust-state or classification schemes beyond DAT-2's `data_origin` + three-value `trust_state`; DLP, CMEK, VPC Service Controls, or Assured Workloads; OTel Collector deployments, custom metrics, or dashboards beyond Cloud Trace's native rendering; any registry write surface beyond the deploy-time load, audited lifecycle/instance transitions, and the HUM-3 operator instance-state controls (the Agent Catalog display stays derived and read-only).
...
- v1.3 — Operator-console amendment per entrant directive 2026-08-23 ("I should have the ability to start/stop workflows and insert anomalies — how will the judges know it actually works"): added HUM-3 (governed operator controls: start, cancel, clock advance, bulletin injection, instance fail/restore — all authenticated, audited, and routed through the sanctioned writers); `CANCELLED` terminal state (workflow_state.v3 per ICD-3); terminal-state guards convert late bus traffic into audited stale no-ops (also closing the pre-existing RELEASED-workflow 500→DLQ gap); §10 amended accordingly; read-only Live Agent Activity feed added to the console (pure AUD-2 projection, no new data). Supersedes the v1.2-demo freeze; re-acceptance (CI-8 10/10) required on the amended tag before recording. No new reasoning agents; scenario otherwise unchanged. Post-amendment corrections under the same ratification (2026-08-24/25, entrant-directed): mission-story/relative-day presentation; monotonic Logical Clock (never reset); registry status coherence (completion returns owners to IDLE; activation audits carry the real day); QA-hold closures (FAILED owners cannot commit late successes; the console enforces one active recovery per vehicle; CI-6 WIF deploy implemented — triggered on acceptance tags and manual dispatch rather than every merge, protecting the frozen runtime (Lane-2 wording aligned); duplicate console starts are refused AND audited (OPERATOR_START_REJECTED); DFT-4 reworded to the tag series).
